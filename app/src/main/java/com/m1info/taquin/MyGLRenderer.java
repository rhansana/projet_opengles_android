...
import com.m1info.taquin.model.Game;
...
public class MyGLRenderer implements GLSurfaceView.Renderer {
    ...
    public void onDrawFrame(GL10 unused) {
       ...
       // Matrix.setLookAtM(mViewMatrix, 0, 0, 0, -3, 0f, 0f, 0f, 0f, 1.0f, 0.0f);
        ...
        this.game.drawBoard(scratch);

        for (Shape shape: this.game.getmGameState().getAllShape()) {
            if(shape != null) {
                float[] shapePos = shape.get_position();
                Matrix.setIdentityM(mModelMatrix, 0);
                Matrix.translateM(mModelMatrix, 0, shapePos[0], shapePos[1], 0);
                Matrix.multiplyMM(scratch, 0, mMVPMatrix, 0, mModelMatrix, 0);
                shape.draw(scratch);
            }
        }
    ...
    public void onSurfaceChanged(GL10 unused, int width, int height) {
         ...
         */
        int width_tel = width/100;
        int height_tel = height/100;
        GLES30.glViewport(0, 0, width, height);
        Matrix.orthoM(mProjectionMatrix, 0, -width_tel, width_tel, -height_tel, height_tel, -1.0f, 1.0f);
